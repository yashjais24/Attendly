...
const Timetable: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingSlot) {
        await fetchWithAuth(`/timetable/${editingSlot.id}`, {
          method: 'PATCH',
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await fetchWithAuth(`/timetable/${id}`, { method: 'DELETE' });
      loadData();
    } catch (err) {
      console.error(err);
  ...
  const handleSplit = async (slot: TimetableSlot) => {
    ...
    const slot1 = { ...slot, endTime: midTime };
    const slot2 = { ...slot, startTime: midTime, lectureNumber: slot.lectureNumber + 1 };
    ...
    try {
      ...
      await fetchWithAuth('/timetable', {
        method: 'POST',
        body: JSON.stringify([
          { subjectId: slot1.subjectId, dayOfWeek: slot1.dayOfWeek, startTime: slot1.startTime, endTime: slot1.endTime, lectureNumber: slot1.lectureNumber, room: slot1.room },
          { subjectId: slot2.subjectId, dayOfWeek: slot2.dayOfWeek, startTime: slot2.startTime, endTime: slot2.endTime, lectureNumber: slot2.lectureNumber, room: slot2.room }
        ])
      });
      // Delete old slot
      await fetchWithAuth(`/timetable/${slot.id}`, { method: 'DELETE' });
      loadData();
    } catch (err) {
    ...
    <div className="p-8 max-w-7xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-1">Manage your recurring weekly schedule.</p>
        </div>
        <button
      ...
      <div className="bg-surface rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="grid grid-cols-6 divide-x divide-gray-100 border-b border-gray-100 bg-gray-50/50">
          {DAYS.map(day => (
            <div key={day} className="py-4 text-center font-semibold text-gray-700">{day}</div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-6 divide-x divide-gray-100 min-h-[60vh] bg-white">
          {[1, 2, 3, 4, 5, 6].map(dayIdx => {
            ...
            return (
              <div key={dayIdx} className="p-3 flex flex-col gap-3 relative">
                {daySlots.map(slot => (
                  <div 
                    key={slot.id} 
                    className="p-3 rounded-xl shadow-sm border transition-all hover:shadow-md group relative cursor-pointer"
                    style={{ 
                      borderColor: slot.subject?.color ? `${slot.subject.color}40` : '#e5e7eb',
                      backgroundColor: slot.subject?.color ? `${slot.subject.color}15` : '#f9fafb',
                    }}
                    onClick={() => handleOpenModal(slot)}
                  >
                    <div className="font-bold text-gray-900 leading-tight mb-1">{slot.subject?.code}</div>
                    <div className="text-xs text-gray-600 font-medium mb-1">{slot.startTime} - {slot.endTime}</div>
                    <div className="text-xs text-gray-500">Lec {slot.lectureNumber} {slot.room && `• ${slot.room}`}</div>
                    ...
                    <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity bg-white/90 p-1 rounded-md shadow-sm">
                       <button onClick={(e) => { e.stopPropagation(); handleSplit(slot); }} className="text-gray-500 hover:text-blue-600 p-1" title="Split Block">
                        <SplitSquareHorizontal size={14} />
                       </button>
                       <button onClick={(e) => { e.stopPropagation(); handleDelete(slot.id); }} className="text-gray-500 hover:text-red-600 p-1" title="Delete">
                        <Trash2 size={14} />
                       </button>
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50">
          <div className="bg-surface w-full max-w-md rounded-2xl shadow-xl overflow-hidden">
            <div className="flex justify-between items-center p-6 border-b border-gray-100">
              <h2 className="text-xl font-bold">{editingSlot ? 'Edit Class' : 'Add Class'}</h2>
